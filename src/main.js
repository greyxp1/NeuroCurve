...
const chartAreaBorder = {
  ...
  beforeDraw(chart) {
    ...
    ctx.shadowBlur = 10;
    const [x1, y1] = getPos(pos), [x2, y2] = getPos((pos + 0.2) % 1),
...
const updatePlot = async (animate = true) => {
  ...
  try {
    const chartData = await invoke('prepare_chart_data', {settings});
    if (chart) {
      chart.data.datasets[0].data = chart.data.datasets[1].data = chartData.y_values;
      chart.data.labels = chartData.x_values;
      chart.options.scales.y.max = chartData.max_y;
      chart.options.scales.x.max = chartData.max_x;
      chart.update({duration: animate ? 300 : 0, easing: 'easeOutCubic', lazy: true});
    } else createChart(chartData.points, chartData.max_y);
  } catch (error) { console.error('Failed to update plot:', error); }
},
...
const createChart = (points, maxValue) => {
  const data = points.map(p => p[1]),
        ...
        baseDataset = {
          data, fill: false, borderWidth: 3.5, pointRadius: 0, borderColor: 'rgba(77,159,255,0.9)',
          backgroundColor: 'transparent', cubicInterpolationMode: 'monotone', tension: 0.4,
  ...
  chart = new Chart($('#sensitivity-plot'), {
    ...
    options: {
      responsive: true, maintainAspectRatio: false, animation: {duration: 300, easing: 'easeOutCubic'},
      plugins: {legend: {display: false}, tooltip: false},
      scales: {
        x: {
          grid: {color: 'rgba(255,255,255,0.02)', borderDash: [3, 3]}, border: {display: false},
          position: 'bottom', type: 'linear', beginAtZero: true, max: Math.max(...points.map(p => p[0])),
          title: {display: true, text: 'Input Speed (counts/ms)', color: 'rgba(255,255,255,0.7)',
                 font: {size: 12, weight: '500'}, padding: {top: 10}},
          ticks: {color: 'rgba(255,255,255,0.5)', font: {size: 11}, padding: 8}
        },
        y: {
          grid: {color: 'rgba(255,255,255,0.02)', borderDash: [3, 3]}, border: {display: false},
          position: 'left', beginAtZero: true, min: 0, max: maxValue,
          title: {display: true, text: 'Sensitivity Multiplier', color: 'rgba(255,255,255,0.7)',
                 font: {size: 12, weight: '500'}, padding: {right: 10}},
          ticks: {
            callback: v => v === maxValue ? '' : Number.isInteger(v) ? v.toString() : v.toString().replace(/\.?0+$/, ''),
...
class SettingsManager {
  ...
  async loadSavedSettings() {
    try {
      ...
      if (appSettings.curve_settings?.values) {
        ...
        Object.entries(settings.values).forEach(async ([key, value]) => {
          const input = $(`#${key}-value`);
          if (input) input.value = await invoke('format_number', { value });
        });
      }
      if (appSettings.raw_accel_settings) {
        this.rawAccelSettings = appSettings.raw_accel_settings;
        Object.entries(this.rawAccelSettings).forEach(async ([key, value]) => {
          const input = $(`#${key}-value`);
          if (input) input.value = await invoke('format_number', { value });
        });
      }
      if (appSettings.raw_accel_path) this.rawAccelPath = appSettings.raw_accel_path;
  ...
  async resetRawAccelSettings() {
    try {
      this.rawAccelSettings = await invoke('reset_raw_accel_settings');
      Object.entries(this.rawAccelSettings).forEach(async ([key, value]) => {
        const input = $(`#${key}-value`);
        if (input) input.value = await invoke('format_number', { value });
      });
      ...
      console.error('Failed to reset raw accel settings:', error);
      return false;
    }
  }
  ...
  initSettings(type, container, keys) {
    ...
    const noValidateKeys = {curve: ['min_sens', 'max_sens'], raw: ['sens_multiplier', 'y_x_ratio', 'growth_base']}[type];
    ...
    keys.forEach(key => {
      ...
      if (type === 'raw') {
        invoke('format_number', { value: info.default }).then(formattedValue => input.value = formattedValue);
      ...
      input.onchange = async e => {
        ...
        if (!isNaN(value)) {
          e.target.value = await invoke('format_number', { value });
  ...
  initCurveSettings() { this.initSettings('curve', '#curve-settings', ['min_sens', 'max_sens', 'offset', 'range', 'growth_base']); }
  initRawAccelSettings() { this.initSettings('raw', '#raw-accel-settings', ['dpi', 'polling_rate', 'sens_multiplier', 'y_x_ratio', 'rotation', 'angle_snapping']); }

  async updateValue(type, key, value) {
    const input = $(`#${key}-value`);
    ...
    const clampedValue = await invoke('update_setting_value', {setting_type: type, key, value, min, max});
    input.value = await invoke('format_number', { value: clampedValue });

    if (type === 'curve' && settings) {
      settings.values[key] = clampedValue;
      debounce(() => updatePlotThrottled(true), 250)();
    } else {
      this.rawAccelSettings[key] = clampedValue;
    }
    this.saveSettings();
  }
  ...
  async handleBrowseClick() {
    try {
      const selected = await window.__TAURI__.dialog.open({directory: true, multiple: false, title: 'Select Raw Accel Directory'});
      if (selected) {
  ...
  getRawAccelSettings() { return { path: this.rawAccelPath, settings: this.rawAccelSettings }; }
}
...
const setupEventListeners = () => {
  ...
  $('#reset-btn').onclick = async () => {
    ...
    try {
      ...
      await settingsManager.resetRawAccelSettings();
  ...
  $('#apply-btn').onclick = async () => {
    ...
    try {
      ...
      await invoke('apply_to_raw_accel', {
        settings, rawAccelPath: settingsManager.rawAccelPath, rawAccelSettings: rawAccelConfig.settings
      });
      await settingsManager.saveSettings();
...
document.head.appendChild(Object.assign(document.createElement('style'), {
  textContent: `
    ::-webkit-validation-bubble-message, ::-webkit-validation-bubble, ::-webkit-validation-bubble-arrow, ::-webkit-validation-bubble-icon,
    ::-moz-validation-bubble-message, ::-moz-validation-bubble, ::-moz-validation-bubble-arrow, ::-moz-validation-bubble-icon {
      display: none !important; opacity: 0 !important; visibility: hidden !important; pointer-events: none !important;
    }
    input:invalid { box-shadow: none !important; outline: none !important; border-color: rgba(255, 255, 255, 0.1) !important; }
    ...
    input::-webkit-strong-password-auto-fill-button, input::-webkit-caps-lock-indicator,
...
const formatNumber = async (n) => {
  try {
    return await invoke('format_number', { value: n });
  } catch (error) {
    console.error('Error using Rust format_number, falling back to JS implementation:', error);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  ['min_sens-value', 'max_sens-value', 'sens_multiplier-value', 'y_x_ratio-value', 'growth_base-value'].forEach(id => {
  ...
  const updateTabOnResize = () => {
    const activeTab = $('.tab-button.active'), tabIndicator = $('.tab-indicator');
  ...
  if (splashScreen) {
    splashScreen.addEventListener('animationend', e => {
      if (e.animationName === 'splashFadeOut') splashScreen.remove();
    });
  }
});
